...
public_metagenome_samples <- samples |>
  filter(SampleID %in% samples$SampleID, 
         ...
         StudyID %in% public_glmr_datasets$dataset, 
...
metal_data %>%
  ...
  labs(x = "Time of Year", 
       y = "RPKM")
ggsave("results/Annual_Variations_of_the_Metal_Resistant_Genes_Microbiome.png", plot = last_plot(), width = 3, height = 2, scale = 2)
ggsave("results/Annual_Variations_of_the_Metal_Resistant_Genes_Microbiome.pdf", plot = last_plot(), width = 3, height = 2, scale = 2)
...
metal_data %>%
  ...
  ggh4x::facet_nested(~ type_name + gene, scales = "free_x")

ggsave("results/BoxIndividual_Trends_of_All_MRGs.png", plot = last_plot(), width = 4, height = 2, scale = 3)
ggsave("results/BoxIndividual_Trends_of_All_MRGs.pdf", plot = last_plot(), width = 4, height = 2, scale = 3)
...
filtered_data <- metal_data %>%
  filter(SampleID %in% public_metagenome_samples$SampleID) %>% 
  dplyr::select(year, rpkm, SampleID, gene) %>% 
  distinct()
...
ggsave("results/Stats_Overall_Trend_of_All_the_Metal_Resistant_Genes.png", plot = plot, width = 4, height = 3, scale = 2)
ggsave("results/Stats_Overall_Trend_of_All_the_Metal_Resistant_Genes.pdf", plot = plot, width = 4, height = 3, scale = 2)


# Linear mixed effects model to account for different gene intercepts
mrg_lme <- lmerTest::lmer(rpkm ~ year + (1| gene),filtered_data)
summary(mrg_lme)

summary_model <- summary(mrg_lme)
year_pval <- summary_model$coefficients["year2019", "Pr(>|t|)"]
year_effect <- summary_model$coefficients["year2019", "Estimate"]


(mrg_lme_plot <- filtered_data %>% 
  ggplot(aes(year, rpkm, color = gene, group = gene)) +
  ggbeeswarm::geom_quasirandom(aes(color = gene), alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE) +
  scale_y_log10() +
  theme_bw() +
  labs(x = "Year", y = "RPKM [log10]") +
  annotate("text", x = -Inf, y = Inf, 
           label = paste0("RPKM ~ year + (1| gene)\nEffect (year) = ", signif(year_effect, 3), 
                          "\nP-value = ", signif(year_pval, 3)), 
           hjust = -0.1, vjust = 1.1, parse = FALSE) +
    guides(color = guide_legend(ncol = 2))
)
  
# Save the plot as PNG and PDF
ggsave("results/Overall_Trend_of_All_ARGs_lme.png", width = 4, height = 3, scale = 1.25)
ggsave("results/Overall_Trend_of_All_ARGs_lme.pdf", width = 4, height = 3, scale = 1.25)
...
other_genes <- c("AmpC", "ermA", "ermC", "TEM-1B", "MerX", "AmpR", "Aac3", "BcrC", "TetA", "TetB", 
                 "Aac(3)-Il", "VanH", "VanA", "VanX", "FosA3", "FosA", "bla", "smeA"
                 # "GyrA", "GyrB", "IpxA" # not ARGs in the same sense as the other genes
                 )
...
ggsave("results/BoxIndividual_Trends_of_All_ARGs.png", plot = last_plot(), width = 4, height = 2, scale = 3)
ggsave("results/BoxIndividual_Trends_of_All_ARGs.pdf", plot = last_plot(),  width = 4, height = 2, scale = 3)
...
other_data %>%
  ...
  labs(x = "Time of Year",
       y = "RPKM")
ggsave("results/Annual_Variations_ARGs_Microbiome.png", plot = last_plot(), width = 3, height = 2, scale = 2)
ggsave("results/Annual_Variations_ARGs_Microbiome.pdf", plot = last_plot(), width = 3, height = 2, scale = 2)
...
# Save the plot as PNG and PDF
ggsave("results/FStats_Overall_Trend_of_All_ARGs.png", plot = plot, width = 4, height = 3, scale = 2)
ggsave("results/FStats_Overall_Trend_of_All_ARGs.pdf", plot = plot, width = 4, height = 3, scale = 2)


# Linear mixed effects model to account for different gene intercepts
arg_lme <- lmerTest::lmer(rpkm ~ year + (1| gene),other_data)
summary(arg_lme)

summary_model <- summary(arg_lme)
year_pval <- summary_model$coefficients["year2019", "Pr(>|t|)"]
year_effect <- summary_model$coefficients["year2019", "Estimate"]


(arg_lme_plot <- other_data %>% 
  ggplot(aes(year, rpkm, color = gene, group = gene)) +
  ggbeeswarm::geom_quasirandom(aes(color = gene), alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE) +
  scale_y_log10() +
...
genes_to_include <- other_genes <- c("AmpC", "ermA", "ermC", "TEM-1B", "MerX", "AmpR", "Aac3", "BcrC", "TetA", "TetB", 
                 "Aac(3)-Il", "VanH", "VanA", "VanX", "FosA3", "FosA", "bla", "smeA"
                 # "GyrA", "GyrB", "IpxA" # not ARGs in the same sense as the other genes
                 )
...
colors_df <- data.frame(color_hex = c("#3182BD", "#31A354", "#E6550D", "#FD8D3C", "#FDD0A2", "#74C476", "#6A51A3", "#B8E186","#9ECAE1", "#C7E9C0", "#FEE6CE" 
                                      #"#FDCDAC", "#FFF7BC", "#D9F0D3", "#DADAEB", "#A1D99B"
                                      ) %>% rev()) %>% 
  mutate(rank = row_number()) %>% 
...
ggmap(google_map) +
  geom_point(data = gene_data_w_colors, 
             aes(lon, lat, color = color_hex, size = rpkm), 
  ...
  coord_quickmap(xlim = c(-83.5, -82.9), ylim = c(41.6, 42.089)) +
  facet_wrap(~type_name,ncol = 1)

# Save the map to a file
ggsave("results/All_MRGs_map.pdf", width = 4, height = 7, scale = 1) 
ggsave("results/All_MRGs_map.png", width = 4, height = 6, scale = 1) 
...
ggmap(google_map) +
  ...
  scale_size_continuous(range = c(.1, 3)) + # Adjust size scale for better visibility
  ...
  theme(legend.position = "right",
        ...
        legend.key.height = unit(0.43, "cm"),
        legend.text = element_text(size = 8)) +
  labs(x = "Longitude", y = "Latitude", color = "Gene", size = "Gene abundance\n(rpkm)") +
  coord_quickmap(xlim = c(-83.5, -82.9), ylim = c(41.6, 42.089)) +
...
ggplot(gene_year_summary, aes(x = gene, y = percentage, fill = factor(year))) + 
  ...
  theme_bw() +
  labs(x = "Gene", y = "% of samples with gene present", fill = "Year") +
  scale_fill_manual(values = c("2014" = "purple", "2019" = "pink")) + # Optional: Set custom colors for the bars
  theme(axis.text.x = element_text(angle = -35, vjust = 1, hjust = 0)) + # Rotate x labels for readability
  facet_grid(~type_name,scales = "free_x",space = "free_x")

# Save the plots
ggsave("results/Percentage_of_Samples_Containing_MRGs_in_2014_and_2019.pdf",  width = 3, height = 1, scale = 2.75)
ggsave("results/Percentage_of_Samples_Containing_MRGs_in_2014_and_2019.png",  width = 3, height = 1, scale = 2.75)
...
ggplot(arg_year_summary, aes(x = gene, y = percentage, fill = factor(year))) + 
  ...
  theme(axis.text.x = element_text(angle = -35, vjust = 1, hjust = 0), legend.title = element_blank()) + # Rotate x labels and remove legend title
  facet_grid(~type_name,scales = "free_x",space = "free_x")

# Save the plots
ggsave("results/Percentage_of_Samples_Containing_ARGs_in_2014_and_2019.pdf", width = 5, height = 2, scale = 2.5)
ggsave("results/Percentage_of_Samples_Containing_ARGs_in_2014_and_2019.png", width = 5, height = 2, scale = 2.5)
```
...
genes_of_interest <- c("AmpC", "TEM-1B", "MerX", "AmpR", "Aac3", "BcrC", "TetA", "TetB", 
                 "Aac(3)-Il", "VanH", "VanA", "VanX", "FosA3", "FosA", "bla", "smeA"
                 # "GyrA", "GyrB", "IpxA" # not ARGs in the same sense as the other genes
                 )
...
## Gene correlations
```{r}
rpkm_wide <- rpkm_results %>% 
  filter(SampleID %in% public_metagenome_samples$SampleID,
         !gene %in% c("GyrA", "GyrB", "IpxA")) %>% 
  select(SampleID, gene, rpkm) %>% 
  group_by(SampleID, gene) %>% 
  summarise(rpkm = sum(rpkm)) %>% 
  filter(rpkm > 0) %>% 
  pivot_wider(names_from = gene, values_from = rpkm, values_fill = 0) %>% 
  column_to_rownames("SampleID")

mrg_arg_corr <- cor(rpkm_wide)

cor_clust <- dist(mrg_arg_corr) %>% hclust(method = "average")

plot(cor_clust)
clust_order <- cor_clust$labels[cor_clust$order] %>% 
  data.frame(gene = .) %>% 
  mutate(order = row_number())

mrg_arg_corr_upper <- mrg_arg_corr
mrg_arg_corr_upper[lower.tri(mrg_arg_corr_upper)] <- NA

mrg_arg_corr_long <- mrg_arg_corr %>% 
  as.data.frame() %>% 
  rownames_to_column("gene1") %>% 
  pivot_longer(-gene1, names_to = "gene2", values_to = "r") %>% 
  filter(gene1 != gene2,
         !is.na(r)) %>% 
  left_join(gene_info %>% select(gene1 = "gene", gene1_type = "type_short", gene1_resistance = "type_name")) %>% 
  left_join(gene_info %>% select(gene2 = "gene", gene2_type = "type_short", gene2_resistance = "type_name")) %>% 
  distinct() %>% 
  mutate(cor_type = case_when(gene1_type == gene2_type ~ "within",
                              gene1_type != gene2_type ~ "between",
                              ),
         resistance_type = case_when(gene1_resistance == gene2_resistance ~ "within",
                                     gene1_resistance != gene2_resistance ~ "between")) %>% 
  left_join(clust_order %>% dplyr::rename(gene1 = "gene", gene1_order = "order")) %>% 
  left_join(clust_order %>% dplyr::rename(gene2 = "gene", gene2_order = "order"))

mrg_arg_corr_long %>% 
  ggplot(aes(cor_type, r)) +
  geom_boxplot()

mrg_arg_corr_long %>%
  filter(cor_type == "between") %>% 
  ggplot(aes(reorder(gene1_resistance,gene1_order), reorder(gene2_resistance,gene2_order), fill = r)) +
  geom_tile() +
  theme_bw() +
...
rpkm_resistance <- rpkm_results %>% 
  filter(
    SampleID %in% public_metagenome_samples$SampleID,
         !gene %in% c("GyrA", "GyrB", "IpxA")) %>% 
  select(SampleID, type_name,gene , rpkm) %>% 
  group_by(SampleID, gene) %>% 
  summarise(rpkm = sum(rpkm)) %>% 
  filter(rpkm > 0) %>% 
  left_join(gene_info) %>% 
  group_by(SampleID, type_name) %>% 
  summarise(rpkm = mean(rpkm)) %>% 
  pivot_wider(names_from = type_name, values_from = rpkm, values_fill = 0) %>% 
  column_to_rownames("SampleID")

resistance_corr <- cor(rpkm_resistance,method = "pearson")

resistance_corr_clust <- dist(resistance_corr) %>% hclust(method = "average")

plot(resistance_corr_clust)
clust_order <- resistance_corr_clust$labels[resistance_corr_clust$order] %>% 
  data.frame(resistance = .) %>% 
  mutate(order = row_number())

resistance_corr %>% 
  as.data.frame() %>% 
  rownames_to_column("res1") %>% 
  pivot_longer(-res1, names_to = "res2", values_to = "r") %>% 
  left_join(clust_order %>% dplyr::rename(res1 = "resistance", res1_order = "order")) %>% 
  left_join(clust_order %>% dplyr::rename(res2 = "resistance", res2_order = "order")) %>% 
  filter(res1 != res2) %>% 
  ggplot(aes(reorder(res1,res1_order), reorder(res2,res2_order), fill = r)) + 
  geom_tile() +
  scale_fill_viridis_c() +
  theme_bw() +
  labs(x = NULL, y = NULL) +
...
ggsave("results/resistance_correlation.png", width = 4, height = 3, scale =2)
ggsave("results/resistance_correlation.pdf", width = 4, height = 3, scale =2)

rpkm_summary <- rpkm_results %>% 
  filter(
    SampleID %in% public_metagenome_samples$SampleID,
         !gene %in% c("GyrA", "GyrB", "IpxA")) %>% 
  left_join(gene_info) %>% 
  select(SampleID, type_name, type_short, rpkm) %>% 
  group_by(SampleID, type_name, type_short) %>% 
  summarise(rpkm = mean(rpkm))

mercury_resistance <- rpkm_summary %>% 
  filter(
    SampleID %in% public_metagenome_samples$SampleID
    ) %>% 
  ungroup() %>% 
  filter(type_name == "Mercury") %>% 
  mutate(mercury_abund = rpkm) %>%
  select(SampleID, mercury_abund) 

abs_rpkm_summary <- rpkm_summary %>% 
  filter(
    SampleID %in% public_metagenome_samples$SampleID
    ) %>% 
  filter(type_short == "ARG") %>% 
  left_join(mercury_resistance)

abs_rpkm_summary %>% 
  ggplot(aes(rpkm, mercury_abund, color = type_name)) +
  geom_point(alpha = 0.25) + 
  scale_x_log10() +
  scale_y_log10() +
  theme_bw() + 
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "ARG abundance (mean rpkm) [log10]",
       y = "Mercury resistance gene abundance (mean rpkm) [log10]",
       color = "Antimicrobial resistance")

ggsave("results/mercury_ARG_correlation.png", width = 4, height = 3, scale =2)
ggsave("results/mercury_ARG_correlation.pdf", width = 4, height = 3, scale =2)
